...
try:
    ...
    if(choice == '1'):
        ...
        while(1):
            df = view_menu_db()
            print(df)
            item_id = raw_input('Enter the IDs of the item you want to order (seperate IDs using (,)): ')
            item_id_l = item_id.split(',')
            for o in item_id_l:
                items.append(df.loc[o]['Item'])
                total_amount = total_amount + order_amt(o)  
            more = raw_input('If you want to order more enter 0 else to confirm order enter 1: ')
            if(more == '0'):
                continue
            else:
